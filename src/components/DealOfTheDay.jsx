...
const deals = [
  // Deals array remains the same
];

const DealOfTheDay = () => {
  const getSlidesToScroll = () => (window.innerWidth <= 768 ? 1 : 4);
  ...
  useEffect(() => {
    const handleResize = () => {
      if (emblaApi) emblaApi.reInit({ slidesToScroll: getSlidesToScroll() });
    };
    ...
    <div className="mb-8 relative">
      ...
      <div className="carousel" ref={emblaRef}>
        <div className="carousel-content flex">
          {deals.map((deal) => (
            <div key={deal.id} className="carousel-item pl-4 cursor-pointer" onClick={() => handleDealClick(deal.url)}>
              <div className="card">
                <div className={`relative w-full h-48 ${deal.bgColor}`}>
                  <img src={deal.image} alt={deal.title} className="w-full h-full object-cover mix-blend-overlay" />

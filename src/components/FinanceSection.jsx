  ...
  if (error) {
    ...
    return (
      <section className="container mx-auto px-4 py-8">
        ...
        <div className="text-center text-red-500">Error loading finance data</div>
      </section>
    );
  }
            ...
            <Card className="h-full overflow-hidden hover:shadow-lg transition-shadow">
              <CardContent className="p-3 md:p-4">
                <div className="flex flex-col items-center justify-center">
                  ...
                  {financeData?.categories?.find(cat => cat.name === category.name) && (
                    <p className="text-xs text-gray-500 mt-1">
                      {financeData.categories.find(cat => cat.name === category.name).products.length} products
                    </p>
                  )}
                </div>
              </CardContent>
